<template>
  <div>
    <v-subheader>Customer Detail</v-subheader>
    <div>
        <v-card flat>
        <v-container>
    ...
    <v-layout row wrap>
        ...
        Total Bill ({{ totalQuantity }} tickets)
        <div class="title">{{ totalPrice.toLocaleString('id-ID') }}</div>
        </v-flex>
        <v-flex xs6 text-center>
        <v-btn color="orange" @click="dialogConfirm=true" :disabled="totalPrice==0">
    ...
    methods: {
      ...
      getServices(){
        ...
        calculateBill(){
            ...
            this.totalBill = parseInt(this.totalPrice) 
        },
        pay(){
